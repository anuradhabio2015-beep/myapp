...
# PAGE CONFIG
# ---------------------------------
st.set_page_config(page_title="Bank Nifty Assistant", page_icon="📈", layout="wide")
...
if df.empty:
    st.error("No data fetched for Bank Nifty.")
    st.stop()
...
# ✅ Get the latest (single row)
latest_row = df.iloc[-1]

if bool(latest_row["Buy"]):
    signal = "BUY"
elif bool(latest_row["Sell"]):
    signal = "SELL"
else:
    signal = "HOLD"
